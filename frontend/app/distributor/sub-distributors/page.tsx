...
export default function SubDistributorsPage() {
  ...
  const [creditingSubDistributor, setCreditingSubDistributor] = useState<SubDistributorSummary | null>(null);
  const [statusEditingSubDistributor, setStatusEditingSubDistributor] = useState<SubDistributorSummary | null>(null);
    ...
    },
    {
      ...
      render: (d) => (
        <div className="flex flex-wrap items-center gap-2">
          <button className={secondaryButtonClass} onClick={() => setCreditingSubDistributor(d)} type="button">
            Credit / debit
          </button>
          <button className={secondaryButtonClass} onClick={() => setStatusEditingSubDistributor(d)} type="button">
  ...
  return (
    <DistributorShell>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="grid gap-1">
            ...
            <p className="leading-relaxed text-muted">
              Promote a direct referral to sub-distributor, fund them in bulk at a discount, and manage their account
              &mdash; the same tools admin uses on you, scoped to your own downline.
            </p>
          </div>
          <PromoteDialog />
        </div>
      ...
      {creditingSubDistributor && (
        <AdjustWalletDialog subDistributor={creditingSubDistributor} onClose={() => setCreditingSubDistributor(null)} />
      )}
      {statusEditingSubDistributor && (
        <StatusDialog subDistributor={statusEditingSubDistributor} onClose={() => setStatusEditingSubDistributor(null)} />
      )}
    </DistributorShell>
  );
}
...
function PromoteDialog() {
  ...
  const subDistributorIds = useMemo(() => new Set((subDistributors ?? []).map((d) => d.id)), [subDistributors]);
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (!next) setError(null); }}>
      ...
      <DialogContent title="Promote to sub-distributor" description="Only your own direct referrals are eligible. This takes effect immediately, no approval needed.">
        <form className="grid gap-3" onSubmit={handleSubmit}>
          {candidates.length === 0 ? (
            <p className="rounded-lg border border-line bg-surface px-3 py-2.5 text-sm leading-relaxed text-muted">
              None of your direct referrals are eligible right now &mdash; they may already be sub-distributors, or you
              have no direct referrals yet.
            </p>
          ) : (
            <div className="grid gap-1">
              ...
              <select className={inputClass} id="promote-candidate" onChange={(e) => setSelectedId(e.target.value)} required value={selectedId}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <ActionButton className={primaryButtonClass} disabled={!selectedId} pending={isPromoting} pendingLabel="Promoting" type="submit">
              Promote
            </ActionButton>
          </div>
        </form>
...
function AdjustWalletDialog({ subDistributor, onClose }: { subDistributor: SubDistributorSummary; onClose: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent title={`Adjust ${subDistributor.name}'s wallet`} description="Bulk-credit at a discount, or make a direct debit -- debits require an emailed confirmation code.">
        <div className="inline-flex w-fit rounded-lg border border-line bg-surface p-1">
          <button className={`rounded-md px-3 py-1.5 text-sm font-extrabold ${direction === 'credit' ? 'bg-accent text-white' : 'text-muted hover:text-ink'}`} onClick={() => setDirection('credit')} type="button">
            Bulk credit
          </button>
          <button className={`rounded-md px-3 py-1.5 text-sm font-extrabold ${direction === 'debit' ? 'bg-accent text-white' : 'text-muted hover:text-ink'}`} onClick={() => setDirection('debit')} type="button">
            Debit
          </button>
        </div>
        ...
        {direction === 'credit' ? (
          <form className="grid gap-3" onSubmit={handleBulkCredit}>
            {!settings?.enabled || !settings?.bulkAllocationEnabled ? (
              ...
              <>
                <div className="grid gap-1">
                  ...
                  <input className={inputClass} id="credit-token-amount" min="0.00000001" onChange={(e) => setTokenAmount(e.target.value)} required step="any" type="number" value={tokenAmount} />
                </div>
                <div className="grid gap-1">
                  <label className="text-xs font-bold uppercase text-muted" htmlFor="credit-discount-rate">
                    Discount rate
                  </label>
                  <input className={inputClass} id="credit-discount-rate" max="1" min="0" onChange={(e) => setDiscountRate(e.target.value)} placeholder={defaultBulkDiscountRate} step="0.0001" type="number" value={discountRate} />
                </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <ActionButton className={primaryButtonClass} disabled={!settings?.enabled || !settings?.bulkAllocationEnabled} pending={isAllocating} pendingLabel="Crediting" type="submit">
                Credit sub-distributor
              </ActionButton>
            </div>
          </form>
        ) : (
          <DebitForm reference={reference} setReference={setReference} subDistributorId={subDistributor.id} onDone={onClose} />
        )}
      </DialogContent>
    </Dialog>
  );
...
function DebitForm({
  ...
  const [error, setError] = useState<string | null>(null);
  const [requestOtp, { isLoading: isRequestingOtp }] = useRequestSubDistributorAdjustmentOtpMutation();
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!otpRequestId) {
        const result = await requestOtp({ id: subDistributorId, amount: signedAmount, reference: reference || 'Manual debit' }).unwrap();
        setOtpRequestId(result.otpRequestId);
        return;
      }
      await adjust({ id: subDistributorId, amount: signedAmount, reference: reference || 'Manual debit', otpRequestId, code }).unwrap();
      onDone();
    } catch (err) {
      setError(normalizeErrorMessage(err, otpRequestId ? 'Unable to verify this code.' : 'Unable to debit this sub-distributor.'));
    }
  }
  ...
  if (otpRequestId) {
    return (
      <form className="grid gap-3" onSubmit={handleSubmit}>
        <p className="text-sm leading-relaxed text-muted">We emailed a 6-digit code to confirm this debit.</p>
        <input
          autoFocus
  ...
  return (
    <form className="grid gap-3" onSubmit={handleSubmit}>
      <div className="grid gap-1">
        ...
        <input className={inputClass} id="debit-token-amount" min="0.00000001" onChange={(e) => setAmount(e.target.value)} required step="any" type="number" value={amount} />
      </div>
      <div className="grid gap-1">
        <label className="text-xs font-bold uppercase text-muted" htmlFor="debit-reference">
          Reference / reason
        </label>
        <input className={inputClass} id="debit-reference" maxLength={120} onChange={(e) => setReference(e.target.value)} placeholder="e.g. Correction for over-credit" value={reference} />
      </div>
      {error && (
        <p className="leading-relaxed text-danger" role="alert">
          {error}
...
function StatusDialog({ subDistributor, onClose }: { subDistributor: SubDistributorSummary; onClose: () => void }) {
  const [status, setStatus] = useState<string>(subDistributor.status === 'ACTIVE' ? 'SUSPENDED' : subDistributor.status);
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent title={`Update ${subDistributor.name}'s status`} description="Suspending or blocking revokes their active sessions.">
        <form className="grid gap-3" onSubmit={handleSubmit}>
          <div className="grid gap-1">
            <label className="text-xs font-bold uppercase text-muted" htmlFor="sub-status">
              Status
            </label>
            <select className={inputClass} id="sub-status" onChange={(e) => setStatus(e.target.value)} value={status}>
