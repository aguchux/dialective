        ...
        <section className="grid gap-6 md:grid-cols-2" aria-label="Blog posts">
          {posts.map((post) => <article className="group grid content-start gap-3 overflow-hidden rounded-lg border border-line bg-white" key={post.slug}>
            ...
            <div className="grid gap-3 p-5">
              <div className="flex flex-wrap items-center gap-2 text-sm text-muted">{post.tag && <span className="font-extrabold uppercase text-accent">{post.tag}</span>}<span>{formatDate(post.publishedAt)}</span><span aria-hidden="true">&middot;</span><span>{estimateReadMinutes(post)} min read</span></div>
