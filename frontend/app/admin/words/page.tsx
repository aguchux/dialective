...
type DisabledSubTabKey = (typeof disabledSubTabs)[number]['key'];
...
export default function AdminWordsPage() {
  ...
  const [disabledSubTab, setDisabledSubTab] = useState<DisabledSubTabKey>('words');
      ...
      <div className="grid gap-6">
        <div className="grid gap-2">
          ...
          <p className="leading-relaxed text-muted">
            ...
            the original hand-seeded bank. Disable a word or sentence to pull it out of the
            trainer-facing pool without touching its existing recordings or assignments; delete
            only removes ones with no history left to protect.
          </p>
        </div>
        ...
        {active === 'words' && <WordsTab disabled={false} />}
        {active === 'sentences' && <SentencesTab disabled={false} />}
        {active === 'disabled' && (
          <div className="grid gap-4">
            <div className="flex gap-1">
              {disabledSubTabs.map((tab) => (
                <button
                  key={tab.key}
                  type="button"
                  onClick={() => setDisabledSubTab(tab.key)}
                  className={`rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${
                    disabledSubTab === tab.key
                      ? 'bg-ink text-white'
            ...
            {disabledSubTab === 'words' && <WordsTab disabled />}
            {disabledSubTab === 'sentences' && <SentencesTab disabled />}
          </div>
        )}
      </div>
    </AdminShell>
...
function WordsTab({ disabled }: { disabled: boolean }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, partOfSpeech, disabled]);

  useEffect(() => {
    setSelected(new Set());
  }, [debouncedSearch, partOfSpeech, disabled, page]);
  ...
  async function handleToggleDisabled(id: string, nextDisabled: boolean) {
    ...
    setTogglingId(id);
    try {
      await setWordDisabled({ id, disabled: nextDisabled }).unwrap();
    } catch (err) {
      setError(
        normalizeErrorMessage(err, nextDisabled ? 'Unable to disable word.' : 'Unable to enable word.'),
      );
    } finally {
      setTogglingId(null);
    }
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex flex-wrap gap-2">
                        <ActionButton
                          ...
                          onClick={() => handleToggleDisabled(word.id, !disabled)}
                          pending={togglingId === word.id}
...
function SentencesTab({ disabled }: { disabled: boolean }) {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebouncedValue(search, 300);
  const pageSize = 20;
  ...
  async function handleToggleDisabled(id: string, nextDisabled: boolean) {
    ...
    setTogglingId(id);
    try {
      await setSentenceDisabled({ id, disabled: nextDisabled }).unwrap();
    } catch (err) {
      setError(
        normalizeErrorMessage(
          err,
          nextDisabled ? 'Unable to disable sentence.' : 'Unable to enable sentence.',
        ),
      );
    } finally {
      setTogglingId(null);
  ...
  return (
    <section className="grid gap-4 overflow-hidden rounded-lg border border-line bg-white shadow-[0_2px_8px_rgba(27,31,27,0.05)]">
      ...
      {isLoading ? (
                  ...
                  <tr key={sentence.id}>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex flex-wrap gap-2">
                        <ActionButton
                          className={secondaryButtonClass}
                          onClick={() => handleToggleDisabled(sentence.id, !disabled)}
                          pending={togglingId === sentence.id}
                          pendingLabel={disabled ? 'Enabling' : 'Disabling'}
                          type="button"
                        >
                          {disabled ? 'Enable' : 'Disable'}
        ...
        <p className="p-5 text-muted">
          {debouncedSearch
            ? `No sentences match "${debouncedSearch}".`
            : disabled
              ? 'No disabled sentences.'
              : 'No sentences yet.'}
        </p>
      )}
