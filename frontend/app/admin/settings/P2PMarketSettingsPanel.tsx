...
import { normalizeErrorMessage, useGetAdminP2PSettingsQuery, useUpdateAdminP2PSettingsMutation } from '@/store/api';
...
export function P2PMarketSettingsPanel() {
  ...
  if (isLoading) return <section className="rounded-lg border border-line bg-white p-5">Loading...</section>;
  ...
  return (
    <form className="grid gap-5 rounded-lg border border-line bg-white p-5" onSubmit={submit}>
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Gate marketplace access, trade limits, countdowns, cancellation grace, and dispute settings.</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Toggle label="Enable P2P market" value={form.enabled} onChange={(enabled) => setForm((current) => ({ ...current, enabled }))} />
        <Toggle label="Enable sell offers" value={form.sellOffersEnabled} onChange={(sellOffersEnabled) => setForm((current) => ({ ...current, sellOffersEnabled }))} />
        <Toggle label="Enable buy requests" value={form.buyRequestsEnabled} onChange={(buyRequestsEnabled) => setForm((current) => ({ ...current, buyRequestsEnabled }))} />
        <Toggle label="Require admin OTP for disputes" value={form.adminOtpRequiredForDisputes} onChange={(adminOtpRequiredForDisputes) => setForm((current) => ({ ...current, adminOtpRequiredForDisputes }))} />
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <NumberField label="Max open offers/user" value={form.maxOpenOffersPerUser} onChange={(maxOpenOffersPerUser) => setForm((current) => ({ ...current, maxOpenOffersPerUser }))} />
        <NumberField label="Max open trades/user" value={form.maxOpenTradesPerUser} onChange={(maxOpenTradesPerUser) => setForm((current) => ({ ...current, maxOpenTradesPerUser }))} />
        <Field label="Allowed fiat currencies" value={form.allowedFiatCurrencies} onChange={(allowedFiatCurrencies) => setForm((current) => ({ ...current, allowedFiatCurrencies }))} />
        <Field label="Allowed payment methods" value={form.allowedPaymentMethods} onChange={(allowedPaymentMethods) => setForm((current) => ({ ...current, allowedPaymentMethods }))} />
      </div>

      <button className="min-h-11 rounded-lg bg-accent px-5 font-extrabold text-white disabled:opacity-60" disabled={isSaving} type="submit">
...
function Field({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="grid gap-1 text-sm font-bold">
      {label}
      <input className="min-h-11 rounded-lg border border-line bg-surface px-3 font-normal" value={value} onChange={(event) => onChange(event.target.value)} />
    </label>
  );
}
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return (
    <label className="grid gap-1 text-sm font-bold">
      {label}
      <input className="min-h-11 rounded-lg border border-line bg-surface px-3 font-normal" type="number" value={value} onChange={(event) => onChange(Number(event.target.value))} />
    </label>
  );
}
...
function Toggle({ label, value, onChange }: { label: string; value: boolean; onChange: (value: boolean) => void }) {
  return (
    <label className="flex items-center justify-between gap-3 rounded-lg border border-line bg-surface px-3 py-3 text-sm font-bold">
      {label}
      <input checked={value} className="size-5 accent-[#6F16B9]" onChange={(event) => onChange(event.target.checked)} type="checkbox" />
    </label>
  );
}
