    ...
    <AdminShell>
      <div className="grid gap-6">
        <header className="flex flex-wrap items-end justify-between gap-3">
          <div><h1 className="text-3xl font-black">Blog</h1><p className="mt-1 text-muted">Create, publish, and order editorial content.</p></div>
          <Link className="rounded-lg bg-accent px-4 py-2.5 font-extrabold text-white no-underline hover:bg-accent-dark" href="/admin/blog/new">Create post</Link>
        </header>
        ...
        <section className="overflow-hidden rounded-lg border border-line bg-white" aria-label="Blog posts">
          <div className="hidden grid-cols-[40px_minmax(0,1fr)_130px_150px_180px] gap-3 border-b border-line bg-surface-muted px-4 py-3 text-xs font-extrabold uppercase text-muted md:grid">
            <span /><span>Post</span><span>Status</span><span>Updated</span><span>Actions</span>
          </div>
          {isLoading && <p className="p-5 text-muted">Loading posts...</p>}
          {!isLoading && visible.length === 0 && <p className="p-8 text-center text-muted">No posts in this view.</p>}
          <DndContext onDragEnd={handleDragEnd} sensors={sensors}>
            <SortableContext items={visible.map((post) => post.id)} strategy={verticalListSortingStrategy}>
              {visible.map((post) => (
                <SortablePostRow
                  key={post.id}
                  post={post}
...
function SortablePostRow({ post, dragDisabled, onDelete, onToggle }: { post: BlogPost; dragDisabled: boolean; onDelete: () => Promise<void>; onToggle: () => Promise<void> }) {
  ...
  return (
    <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={`grid gap-3 border-b border-line px-4 py-4 last:border-0 md:grid-cols-[40px_minmax(0,1fr)_130px_150px_180px] md:items-center ${isDragging ? 'relative z-10 bg-white shadow-lg' : ''}`}>
      ...
      <div className="min-w-0"><Link className="font-extrabold text-ink no-underline hover:text-accent" href={`/admin/blog/${post.id}`}>{post.title}</Link><p className="mt-1 truncate text-sm text-muted">/{post.slug}</p></div>
      <span className={`w-fit rounded-md px-2.5 py-1 text-xs font-extrabold ${post.status === 'PUBLISHED' ? 'bg-accent-soft text-accent-dark' : 'bg-surface-muted text-muted'}`}>{post.status === 'PUBLISHED' ? 'Published' : 'Draft'}</span>
