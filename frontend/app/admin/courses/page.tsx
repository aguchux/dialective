...
export default function AdminCoursesPage() {
  ...
  const handleDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    try {
      await reorder({
        items: next.map((course, sortOrder) => ({ id: course.id, sortOrder })),
      }).unwrap();
    } catch (mutationError) {
      setCourses(courses);
      ...
      <div className="grid gap-6">
        <header className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-muted">Create, publish, and order Learning Center courses.</p>
          </div>
          <Link
        ...
        >
          ...
          {!isLoading && visible.length === 0 && (
            <p className="p-8 text-center text-muted">No courses in this view.</p>
          )}
          <DndContext onDragEnd={handleDragEnd} sensors={sensors}>
            <SortableContext
              items={visible.map((course) => course.id)}
                ...
                <SortableCourseRow
                  ...
                  onDelete={async () => {
                    ...
                    try {
                      ...
                      setError(
                        normalizeErrorMessage(mutationError, 'Unable to delete this course.'),
                      );
                    }
                  }}
                  onToggle={async () => {
                    ...
                    try {
                      ...
                      setError(
                        normalizeErrorMessage(mutationError, 'Unable to update this course.'),
                      );
                    }
                  }}
                />
...
function SortableCourseRow({
  ...
  const completionPct =
    course.totalTrainerCount > 0
      ? Math.round((course.completedTrainerCount / course.totalTrainerCount) * 100)
      : 0;
  return (
    <article
      ref={setNodeRef}
      style={{ transform: CSS.Transform.toString(transform), transition }}
      className={`grid gap-3 border-b border-line px-4 py-4 last:border-0 md:grid-cols-[40px_minmax(0,1fr)_130px_140px_150px_180px] md:items-center ${isDragging ? 'relative z-10 bg-white shadow-lg' : ''}`}
    >
      <button
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate text-sm text-muted">
          /{course.slug} &middot; {course.slides.slides.length}{' '}
          {course.slides.slides.length === 1 ? 'slide' : 'slides'}
          {course.required ? ' · Required' : ''}
        </p>
      </div>
      <span
        className={`w-fit rounded-md px-2.5 py-1 text-xs font-extrabold ${course.status === 'PUBLISHED' ? 'bg-accent-soft text-accent-dark' : 'bg-surface-muted text-muted'}`}
      >
      ...
      <div className="text-sm">
        ...
        {course.totalTrainerCount > 0 && (
          <span className="ml-1 text-xs text-muted">({completionPct}%)</span>
        )}
      </div>
      <time className="text-sm text-muted" dateTime={course.updatedAt}>
      ...
      <div className="flex items-center gap-2">
        ...
        <ActionButton
          ...
          pendingLabel={course.status === 'PUBLISHED' ? 'Unpublishing' : 'Publishing'}
          type="button"
        >
