...
const BlogEditor = dynamic(
  () => import('@/components/blog/BlogEditor').then((module) => module.BlogEditor),
  {
    ssr: false,
    loading: () => (
      <div className="min-h-40 rounded-lg border border-line bg-white p-4 text-sm text-muted">
        Loading editor...
      </div>
    ),
  },
...
const fieldClass =
  'min-h-11 w-full rounded-lg border border-line bg-white px-3 py-2 text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/15';
...
export function CourseEditorForm({ course }: { course?: Course }) {
  ...
  const [completionRewardTokens, setCompletionRewardTokens] = useState(
  ...
  const [uploadingSlideKey, setUploadingSlideKey] = useState<{
    key: string;
    kind: 'image' | 'audio';
  } | null>(null);
  ...
  const onReadyCallbacksRef = useRef<Map<string, (handle: BlogEditorHandle | null) => void>>(
    new Map(),
  );
  ...
  const uploadMedia = useCallback(
    async (file: File, kind: 'IMAGE' | 'AUDIO') => {
      ...
      if (file.size > maxBytes)
        throw new Error(
          kind === 'AUDIO'
            ? 'Audio files must be 15 MB or smaller'
  ...
  const uploadSlideMedia = useCallback(
  ...
  function updateSlide(key: string, patch: Partial<CourseSlide>) {
    setSlides((current) =>
      current.map((entry) =>
        entry.key === key ? { key, slide: { ...entry.slide, ...patch } } : entry,
      ),
    );
  }
  ...
  async function handleSlideAudio(key: string, file?: File) {
    ...
    try {
      ...
      setError(
        uploadError instanceof Error ? uploadError.message : 'Slide narration upload failed',
      );
    } finally {
      setUploadingSlideKey(null);
    }
  ...
  return (
    <div className="grid gap-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {course?.status === 'PUBLISHED' && (
            <Link
              className="rounded-lg border border-line bg-white px-4 py-2 font-bold text-ink no-underline hover:bg-surface-muted"
          ...
          {course && (
            <Dialog>
              <DialogTrigger
                className="min-h-10 rounded-lg border border-line bg-white px-4 py-2 font-bold text-danger transition-colors hover:bg-[#fde8e8]"
                type="button"
              >
                Delete
              </DialogTrigger>
              <DialogContent
                ...
                description="This permanently removes the course and its slides. This cannot be undone."
              >
                <div className="flex justify-end gap-2">
                  <DialogClose
                    className="min-h-10 rounded-lg border border-line bg-white px-4 py-2 font-bold text-ink hover:bg-surface-muted"
                    type="button"
                  >
                    Cancel
                  </DialogClose>
          ...
          <ActionButton
            className="min-h-10 rounded-lg bg-accent px-5 py-2 font-extrabold text-white disabled:opacity-60"
      ...
      {error && (
        <p
          className="rounded-lg border border-[#e8b8bd] bg-[#fff1f2] px-4 py-3 font-semibold text-[#a3242f]"
          role="alert"
        >
          {error}
        </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="grid content-start gap-5">
          <section className="grid gap-4 rounded-lg border border-line bg-white p-5">
            <label className="grid gap-1.5 font-bold">
              Title
              <input
                className={`${fieldClass} text-xl font-extrabold`}
                maxLength={180}
            ...
            <p className="text-sm text-muted">
              ...
              <span className="font-mono">
                {course ? `${slugPreview || 'course'}` : `${toSlug(title) || 'course'}-<id>`}
              </span>{' '}
          ...
          <section className="grid gap-4 rounded-lg border border-line bg-white p-5">
            <div className="flex items-center justify-between">
              ...
              <button
                className="rounded-lg border border-line bg-white px-3 py-1.5 text-sm font-bold hover:bg-surface-muted"
                onClick={addSlide}
                type="button"
              >
                + Add slide
              ...
              <div className="grid gap-3 rounded-lg border border-line bg-surface p-4" key={key}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="grid size-8 place-items-center rounded-md text-muted hover:bg-surface-muted disabled:cursor-not-allowed disabled:opacity-30"
                      disabled={index === 0}
                    ...
                    <button
                      ...
                      className="grid size-8 place-items-center rounded-md text-[#a3242f] hover:bg-[#fff1f2] disabled:cursor-not-allowed disabled:opacity-30"
                ...
                <div className="grid gap-1.5 text-sm font-bold">
                  ...
                  <SlideTextEditor
                    ...
                    onReady={getOnReadyCallback(key)}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="grid gap-1.5">
                    ...
                    <label className="grid min-h-16 cursor-pointer place-items-center rounded-lg border border-dashed border-line bg-white px-3 text-center text-sm font-bold text-muted hover:border-accent">
                      {uploadingSlideKey?.key === key && uploadingSlideKey.kind === 'image' ? (
                      ...
                      <input
                        ...
                        disabled={anyUploading}
                        onChange={(event) => handleSlideImage(key, event.target.files?.[0])}
                        type="file"
                      />
                    </label>
                  </div>
                  <div className="grid gap-1.5">
                    ...
                    <label className="grid min-h-16 cursor-pointer place-items-center rounded-lg border border-dashed border-line bg-white px-3 text-center text-sm font-bold text-muted hover:border-accent">
                      {uploadingSlideKey?.key === key && uploadingSlideKey.kind === 'audio' ? (
                        <span className="inline-flex items-center gap-2">
                          <ActionSpinner />
                          Uploading
                        </span>
                      ...
                      <input
                        ...
                        disabled={anyUploading}
                        onChange={(event) => handleSlideAudio(key, event.target.files?.[0])}
                        type="file"
                      />
                    </label>
                  </div>
        ...
        <aside className="grid content-start gap-5">
          <section className="grid gap-3 rounded-lg border border-line bg-white p-4">
            ...
            <div
              className="grid grid-cols-2 rounded-lg border border-line bg-surface-muted p-1"
              role="group"
              aria-label="Course status"
            >
              {(['DRAFT', 'PUBLISHED'] as const).map((option) => (
                <button
                  className={`rounded-md px-3 py-2 text-sm font-extrabold ${status === option ? 'bg-white text-accent shadow-sm' : 'text-muted'}`}
                  key={option}
          ...
          <section className="grid gap-3 rounded-lg border border-line bg-white p-4">
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-line bg-surface-muted p-3">
              ...
              <span>
                ...
                <span className="mt-0.5 block text-xs leading-relaxed text-muted">
                  Trainers must complete this course before they can start a training session or
                  submit a task. Only enforced once this course is Published.
                </span>
              </span>
            </label>
            ...
            <label className="grid gap-1.5 text-sm font-bold">
              ...
              <span className="text-xs font-normal leading-relaxed text-muted">
                Credited once to a trainer&rsquo;s DL balance the first time they complete this
                course. Leave blank for no reward -- trainers still get a completion email either
                way.
              </span>
            </label>
          </section>
          ...
          <section className="grid gap-2 rounded-lg border border-line bg-white p-4">
            ...
            <p className="truncate text-sm text-[#2463c5]">
              dialectlibrary.com/learn/
              {course ? slugPreview || 'course' : `${toSlug(title) || 'course'}-...`}
            </p>
            <p className="font-extrabold leading-snug">{title || 'Course title'}</p>
            <p className="text-sm leading-relaxed text-muted">
              {summary || 'A short description shown on the catalog card.'}
            </p>
          </section>
        </aside>
      </div>
