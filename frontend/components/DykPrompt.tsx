...
export function DykPrompt() {
  ...
  const eligible =
    ...
    ['TRAINER', 'DISTRIBUTOR'].includes(session?.user?.role ?? '') &&
    /^\/(dashboard|distributor)(\/|$)/.test(pathname);
...
function ActivePrompt() {
    ...
    const check = async () => {
      ...
      try {
        ...
        if (
          !alive.current ||
          !result.items.length ||
          (result.nextAt && Date.parse(result.nextAt) > Date.now())
        )
          return;
        if (document.hidden || document.querySelector('[role="dialog"], [role="alertdialog"]'))
          return;
        ...
        /* Optional notices must never block dashboard use. */
      } finally {
        checking = false;
      }
    };
  ...
  const navigate = async (step: number) => {
    ...
    try {
      ...
      setError('Unable to load the next notice. Please try again.');
    } finally {
      setBusy(false);
    }
  };
  const follow = async () => {
    ...
    try {
      ...
      setError('Unable to open this notice. Please try again.');
    } finally {
      setBusy(false);
    }
  };
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[81] w-[min(94vw,960px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg bg-neutral-950 text-white shadow-2xl focus:outline-none">
          <div className="relative aspect-[3/2] min-h-[360px] max-h-[90dvh] overflow-y-auto sm:min-h-0">
            ...
            <div className="relative flex min-h-full flex-col justify-end bg-gradient-to-t from-black via-black/50 to-transparent px-12 pb-8 pt-20 sm:px-20 sm:pb-12">
              ...
              <Dialog.Description className="mt-3 whitespace-pre-wrap break-words text-sm leading-relaxed text-white sm:text-lg">
                {notice.content}
              </Dialog.Description>
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <button
                  disabled={busy}
                  onClick={follow}
                  className="flex min-h-11 items-center gap-2 rounded-md bg-purple-700 px-5 py-2 font-bold text-white disabled:opacity-60"
                >
                ...
                <button
                  onClick={close}
                  className="min-h-11 rounded-md px-3 py-2 font-semibold hover:bg-white/10"
                >
                  Not now
                </button>
              </div>
            ...
            {items.length > 1 && (
              <>
                <button
                  ...
                  disabled={busy}
                  onClick={() => navigate(-1)}
                  className="absolute left-2 top-1/3 grid size-9 place-items-center rounded-full bg-black/60 disabled:opacity-50 sm:left-4 sm:size-11"
                >
                  <ChevronLeft />
                </button>
                <button
                  ...
                  disabled={busy}
                  onClick={() => navigate(1)}
                  className="absolute right-2 top-1/3 grid size-9 place-items-center rounded-full bg-black/60 disabled:opacity-50 sm:right-4 sm:size-11"
                >
                  <ChevronRight />
                </button>
              </>
          ...
          <span
            aria-live="polite"
            className="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs text-white/80"
          >
            {index + 1} / {items.length}
          </span>
          <Dialog.Close
            aria-label="Close notice"
            className="absolute right-3 top-3 grid size-10 place-items-center rounded-full bg-black/50"
          >
            <X />
          </Dialog.Close>
        </Dialog.Content>
