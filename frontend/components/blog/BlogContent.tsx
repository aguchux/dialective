  ...
  return (
    <div className="blog-prose">
      {blocks.map((block, index) => (
        <BlogBlock block={block} key={block.id ?? `${block.type}-${index}`} />
      ))}
    </div>
  );
}
...
function BlogBlock({ block }: { block: PublicEditorBlock }) {
  ...
  switch (block.type) {
      ...
      return (
        <ul className="blog-checklist">
          {(Array.isArray(data.items) ? data.items : []).map((item, index) => {
            ...
            return (
              <li key={index}>
                <span aria-hidden="true" className={entry.checked ? 'is-checked' : ''}>
                  {entry.checked ? '\u2713' : ''}
                </span>
                <div dangerouslySetInnerHTML={{ __html: inline(entry.text) }} />
              </li>
            );
          })}
        </ul>
    ...
    case 'quote':
      return (
        <blockquote>
          <div dangerouslySetInnerHTML={{ __html: inline(data.text) }} />
          {data.caption ? (
            <cite dangerouslySetInnerHTML={{ __html: inline(data.caption) }} />
          ) : null}
        </blockquote>
      );
    case 'table':
    ...
    case 'video': {
      ...
      return (
        <figure>
          <video className="blog-media" controls preload="metadata" src={url} />
          {data.caption ? (
            <figcaption dangerouslySetInnerHTML={{ __html: inline(data.caption) }} />
          ) : null}
        </figure>
        ...
        <figure>
          <div className="blog-embed">
            <iframe
              ...
              loading="lazy"
              src={url}
              title={plain(data.caption) || 'Embedded video'}
            />
          </div>
          {data.caption ? (
            <figcaption dangerouslySetInnerHTML={{ __html: inline(data.caption) }} />
    ...
    case 'raw':
      return (
        <div
          dangerouslySetInnerHTML={{ __html: sanitizeHtml(String(data.html ?? ''), rawSanitize) }}
        />
      );
    case 'button': {
      ...
      if (!href) return null;
      return (
        <p className="blog-button-row">
          <a className={data.variant === 'secondary' ? 'secondary' : ''} href={href}>
            {plain(data.text) || 'Learn more'}
          </a>
        </p>
      );
    }
      ...
      {items.map((item, index) => {
        const nested =
          typeof item === 'object' && item
            ? (item as { content?: unknown; items?: unknown })
            : null;
        ...
        return (
          <li key={index}>
            ...
            {children.length > 0 && <RichList items={children} ordered={ordered} />}
          </li>
        );
      })}
    </Tag>
...
function inline(value: unknown) {
  return sanitizeHtml(String(value ?? ''), inlineSanitize);
}
...
function safeEmbedUrl(value: unknown) {
  ...
  return ['youtube.com', 'youtube-nocookie.com', 'player.vimeo.com'].includes(host) ? url : null;
}
