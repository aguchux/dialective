...
const BlogEditor = dynamic(() => import('./BlogEditor').then((module) => module.BlogEditor), {
  ssr: false,
  loading: () => (
    <div className="min-h-105 rounded-lg border border-line bg-white p-6 text-muted">
      Loading editor...
    </div>
  ),
});
...
export function BlogEditorForm({ post }: { post?: BlogPost }) {
  ...
  const uploadMedia = useCallback(
    async (file: File, kind: 'IMAGE' | 'VIDEO') => {
      ...
      if (file.size > maxBytes)
        throw new Error(
          kind === 'VIDEO' ? 'Videos must be 250 MB or smaller' : 'Images must be 10 MB or smaller',
        );
  ...
  const handleEditorReady = useCallback(
    (handle: BlogEditorHandle | null) => setEditorHandle(handle),
    [],
  );
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <ActionButton
            ...
            disabled={isUploadingCover || !editorHandle}
            onClick={save}
            pending={isSaving}
          ...
          <section className="grid gap-4 rounded-lg border border-line bg-white p-5">
            ...
            <p className="text-sm text-muted">
              ...
              <span className="font-mono">
                {post ? `${slugPreview || 'post'}` : `${toSlug(title) || 'post'}-<id>`}
              </span>{' '}
        ...
        <aside className="grid content-start gap-5">
          ...
          <section className="grid gap-2 rounded-lg border border-line bg-white p-4">
            ...
            <p className="truncate text-sm text-[#2463c5]">
              dialectlibrary.com/blog/
              {post ? slugPreview || 'post' : `${toSlug(title) || 'post'}-...`}
            </p>
            <p className="font-extrabold leading-snug">{title || 'Post title'}</p>
            <p className="text-sm leading-relaxed text-muted">
              {seoExcerpt || 'The first paragraph becomes the search description.'}
            </p>
          </section>
        </aside>
      </div>
...
function excerptFrom(document: EditorDocument): string {
  const block = document.blocks.find(
    (item) => item.type === 'paragraph' && typeof item.data.text === 'string' && item.data.text,
  );
