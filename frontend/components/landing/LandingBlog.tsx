  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {posts.map((post) => (
          <article
            className="group grid content-start gap-3 overflow-hidden rounded-lg border border-[rgba(5,5,5,0.1)] bg-white/80"
            key={post.slug}
          >
            {post.coverImageUrl && (
              <Link
            ...
            <div className="grid gap-2 p-4">
              <h3 className="text-lg font-extrabold leading-snug">
                <Link
                  className="text-[#050505] no-underline hover:text-accent"
                  href={`/blog/${post.slug}`}
                >
                  {post.title}
                </Link>
              ...
              <time className="text-xs text-muted" dateTime={post.publishedAt ?? undefined}>
