...
export function EmptyPanel({
  ...
  return (
    <div
      className={`${unframed ? '' : cardClass} grid min-h-48 place-items-center p-6 text-center`}
    >
      <div className="grid justify-items-center gap-3">
        ...
        {actionHref && actionLabel && (
          <Link
            className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-line px-3 font-extrabold text-ink hover:bg-surface-muted"
            href={actionHref}
          >
            {actionLabel}
...
export function Avatar({
  ...
  return (
    <span
      className={`${size} grid shrink-0 place-items-center rounded-full bg-accent font-black text-white`}
      aria-hidden="true"
    >
