...
export async function getPublishedBlogPosts(): Promise<PublicBlogPost[]> {
  const response = await fetch(`${PUBLIC_API_V1_BASE_URL}/blog/posts`, {
    next: { revalidate: 60 },
  });
...
export async function getPublishedBlogPost(slug: string): Promise<PublicBlogPost | null> {
  const response = await fetch(`${PUBLIC_API_V1_BASE_URL}/blog/posts/${encodeURIComponent(slug)}`, {
    next: { revalidate: 60 },
  });
...
export function estimateReadMinutes(post: PublicBlogPost): number {
  ...
  const text = JSON.stringify(post.content?.blocks ?? [])
    .replace(/<[^>]*>/g, ' ')
    .replace(/[^a-zA-Z0-9']+/g, ' ');
