...
import { u32be, nu64be, struct, seq, blob, offset, Layout } from '@solana/buffer-layout';
...
function createBurnIntent(params: { depositorAddress: string; recipientAta: string }) {
...
async function main() {
  ...
  const response = await fetch('https://gateway-api-testnet.circle.com/v1/transfer', {
    ...
    body: stringifyTypedData([{ burnIntent: typedData.message, signature: burnIntentSignature }]),
  });

  if (!response.ok) {
    throw new Error(`Gateway API request failed: ${response.status} ${await response.text()}`);
  }
