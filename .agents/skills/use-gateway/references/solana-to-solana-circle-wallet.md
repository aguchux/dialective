...
async function signAndBroadcast(transaction: Transaction, label: string): Promise<string> {
...
async function main() {
  ...
  const response = await fetch('https://gateway-api-testnet.circle.com/v1/transfer', {
    ...
    body: JSON.stringify([{ burnIntent, signature: formattedSignature }], (_key, value) =>
