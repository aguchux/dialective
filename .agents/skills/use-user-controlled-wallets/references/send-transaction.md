...
**On Arc, the native asset IS USDC** (`0x3600000000000000000000000000000000000000`). The empty-`tokenAddress` "native" balance and the USDC ERC-20 balance are the _same_ pool of funds, not two assets — never show or sum them as separate balances. For USDC app logic on Arc, use the ERC-20 view (6 decimals).
