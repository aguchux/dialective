...
export class SettingsController {
  ...
  updateSettings(@Body() dto: UpdatePlatformSettingsDto) {
    // trainingEconomyEnabled is deliberately NOT settable here. It is one
    // atomic step-up-guarded action, not a field on a bulk form -- see
    // TrainingEconomyController. The DTO omits it, so a client sending it
    // is rejected by the global ValidationPipe's forbidNonWhitelisted.
