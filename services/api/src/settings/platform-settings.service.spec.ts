...
describe('PlatformSettingsService.getTawkToWidget', () => {
  it('returns disabled with no IDs when the master switch is off, even if IDs are set', async () => {
    const { service } = setup({ tawkToEnabled: false, tawkToPropertyId: 'prop-1', tawkToWidgetId: 'widget-1' });

    await expect(service.getTawkToWidget()).resolves.toEqual({ enabled: false, propertyId: null, widgetId: null });
  });

  it('returns disabled with no IDs when enabled but propertyId is missing', async () => {
    const { service } = setup({ tawkToEnabled: true, tawkToPropertyId: null, tawkToWidgetId: 'widget-1' });

    await expect(service.getTawkToWidget()).resolves.toEqual({ enabled: false, propertyId: null, widgetId: null });
  });

  it('returns disabled with no IDs when enabled but widgetId is missing', async () => {
    const { service } = setup({ tawkToEnabled: true, tawkToPropertyId: 'prop-1', tawkToWidgetId: null });

    await expect(service.getTawkToWidget()).resolves.toEqual({ enabled: false, propertyId: null, widgetId: null });
  });

  it('returns enabled with both IDs only when the switch is on and both IDs are set', async () => {
    const { service } = setup({ tawkToEnabled: true, tawkToPropertyId: 'prop-1', tawkToWidgetId: 'widget-1' });
