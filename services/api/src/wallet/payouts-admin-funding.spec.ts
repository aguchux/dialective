...
function setupWallet(existingWallet?: { id: string; userId: string; balance: InstanceType<typeof Decimal> }) {
  const wallets = new Map<string, { id: string; userId: string; balance: InstanceType<typeof Decimal> }>();
  ...
  const prisma = {
    wallet: {
      ...
      update: jest.fn(({ where, data }: { where: { id: string }; data: { balance: { increment: InstanceType<typeof Decimal> } | InstanceType<typeof Decimal> } }) => {
        const wallet = [...wallets.values()].find((w) => w.id === where.id);
        if (wallet) {
          wallet.balance = 'increment' in data.balance ? wallet.balance.add(data.balance.increment) : data.balance;
        }
        return Promise.resolve(wallet);
      }),
      updateMany: jest.fn(({
    ...
    ledgerEntry: {
      findFirst: jest.fn(({ where }: { where: { walletId: string; type: string } }) =>
        Promise.resolve(ledgerEntries.find((entry) => entry.walletId === where.walletId && entry.type === where.type) ?? null),
      ),
...
describe('creditAdminFunding', () => {
  it('credits the wallet and writes an ADMIN_FUNDING ledger entry, creating the wallet if missing', async () => {
    ...
    const result = await creditAdminFunding(prisma, 'user-1', 250, 'Manual correction for missed payout');

    expect(result).toEqual({ userId: 'user-1', reference: 'Manual correction for missed payout', amount: '250' });
    expect(wallets.get('user-1')!.balance.toString()).toBe('250');
    expect(ledgerEntries).toEqual([
      expect.objectContaining({ type: 'ADMIN_FUNDING', reference: 'Manual correction for missed payout' }),
    ]);
    expect(ledgerEntries[0].amount.toString()).toBe('250');
  });

  it('adds to an existing balance rather than replacing it', async () => {
    const { prisma, wallets } = setupWallet({ id: 'wallet-user-1', userId: 'user-1', balance: new Decimal('40') });
...
describe('adjustAdminWallet', () => {
  it('debits the wallet silently and writes a signed ADMIN_ADJUSTMENT ledger entry', async () => {
    const { prisma, wallets, ledgerEntries } = setupWallet({ id: 'wallet-user-1', userId: 'user-1', balance: new Decimal('40') });

    const result = await adjustAdminWallet(prisma, 'user-1', -15, 'duplicate signup bonus correction');
    ...
    expect(ledgerEntries).toEqual([
      expect.objectContaining({ type: 'ADMIN_ADJUSTMENT', reference: 'duplicate signup bonus correction' }),
    ]);
    expect(ledgerEntries[0].amount.toString()).toBe('-15');
  });

  it('rejects a debit that would overdraw the wallet', async () => {
    const { prisma, wallets, ledgerEntries } = setupWallet({ id: 'wallet-user-1', userId: 'user-1', balance: new Decimal('10') });

    await expect(adjustAdminWallet(prisma, 'user-1', -15, 'overdraw')).rejects.toThrow('Insufficient wallet balance for this debit');
