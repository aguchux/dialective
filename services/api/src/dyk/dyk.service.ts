...
export class DykService {
  ...
  private present<T extends { imageBucket: string; imageKey: string }>(notice: T) {
    return {
      ...notice,
      imageUrl: this.storage.getPublicObjectUrl(notice.imageBucket, notice.imageKey),
    };
  }

  async listAdmin() {
    return (
      await this.prisma.dykNotice.findMany({
  ...
  async saveNotice(dto: DykNoticeDto, id?: string) {
    ...
    try {
      ...
      /* Internal routes are not absolute URLs. */
    }
    if (!internal && !external)
      throw new BadRequestException('Choose an internal route or approved HTTPS channel');
    ...
    if (
      wantsCourse &&
      !(await this.prisma.course.findUnique({ where: { id: dto.targetId || '' } }))
    ) {
  ...
  private async adopted(
    ...
    for (const condition of notice.stopConditions) {
      const met = await (async () => {
        switch (condition) {
          case 'CLICKED':
            return !!state?.clickedAt;
          case 'VISITED':
            return !!state?.visitedAt;
          case 'PHONE':
            return !!(
              await tx.user.findUnique({ where: { id: userId }, select: { phoneVerifiedAt: true } })
            )?.phoneVerifiedAt;
          case 'KYC':
            return (
              (await tx.user.findUnique({ where: { id: userId }, select: { kycStatus: true } }))
                ?.kycStatus === 'APPROVED'
            );
          case 'PWA':
            return !!(
              await tx.user.findUnique({ where: { id: userId }, select: { pwaInstalledAt: true } })
            )?.pwaInstalledAt;
          case 'REFERRAL_SHARE':
            return !!(await tx.marketingCampaignShare.findFirst({
              where: { userId },
              select: { id: true },
            }));
          case 'TRAINING':
            return !!(await tx.wordRecording.findFirst({
              where: { userId },
              select: { id: true },
            }));
          case 'TESTIMONY':
            return !!(await tx.testimony.findFirst({ where: { userId }, select: { id: true } }));
          case 'QRAC':
            return !!(await tx.qracAffirmationSubmission.findFirst({
              where: { userId },
              select: { id: true },
            }));
          case 'COURSE':
            return !!(await tx.courseProgress.findFirst({
              where: { userId, courseId: notice.targetId ?? '', completedAt: { not: null } },
              select: { id: true },
            }));
          default:
            return false;
  ...
  async feed(userId: string) {
    ...
    const notices = await this.prisma.dykNotice.findMany({
      where: { active: true },
      orderBy: [{ sortOrder: 'asc' }, { createdAt: 'asc' }],
      include: { states: { where: { userId } } },
    });
    const latest = await this.prisma.dykUserState.findFirst({
      where: { userId, lastShownAt: { not: null } },
    ...
    for (const { states, ...notice } of notices) {
      ...
      if (
        state?.lastShownAt &&
        Date.now() - state.lastShownAt.getTime() < settings.intervalMinutes * 60000
      )
        continue;
    ...
    return {
      items,
      nextAt: latest?.lastShownAt
        ? new Date(latest.lastShownAt.getTime() + settings.intervalMinutes * 60000).toISOString()
        : null,
    };
  }
  ...
  async impression(userId: string, noticeId: string, navigation = false) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const state = await tx.dykUserState.findUnique({
      ...
      if (!navigation) {
        const latest = await tx.dykUserState.findFirst({
          where: { userId, lastShownAt: { gt: new Date(cutoff) } },
        });
