...
describe('audit completeness', () => {
    ...
    describe('StreamAccessLog: every manifest/audio request is logged regardless of outcome', () => {
        it('a successful manifest request produces an "allowed" StreamAccessLog row', async () => {
            ...
            const before = await prisma.streamAccessLog.count({ where: { organizationId: org.organizationId } });
            const res = await (0, supertest_1.default)(app.getHttpServer())
            ...
            await (0, fixtures_1.waitForCondition)(async () => {
                const count = await prisma.streamAccessLog.count({ where: { organizationId: org.organizationId } });
                return count > before;
            });
            const after = await prisma.streamAccessLog.count({ where: { organizationId: org.organizationId } });
        ...
        it('an audio stream request (allowed or denied) always produces a StreamAccessLog row', async () => {
            ...
            const res = await (0, supertest_1.default)(app.getHttpServer())
                .get((0, test_app_1.apiPath)(`stream/v1/decks/${deck.deckId}/items/00000000-0000-0000-0000-000000000000/audio`))
                .set('Authorization', `Bearer ${key.plaintextKey}`);
