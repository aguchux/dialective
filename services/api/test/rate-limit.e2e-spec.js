...
describe('rate limiting', () => {
    ...
    describe('StreamKeyRateLimitGuard tracks per-key, not per-org', () => {
        it('exhausting one key does not affect a second key from the same org', async () => {
            const orgWithTightLimit = await (0, fixtures_1.seedOrg)(prisma, { planOverrides: { rateLimitPerMinute: 1 } });
            orgIds.push(orgWithTightLimit.organizationId);
            const deck = await (0, fixtures_1.seedDeck)(prisma, orgWithTightLimit.organizationId, orgWithTightLimit.userId);
            const keyA = await (0, fixtures_1.seedStreamKey)(prisma, orgWithTightLimit.organizationId, orgWithTightLimit.userId);
            const keyB = await (0, fixtures_1.seedStreamKey)(prisma, orgWithTightLimit.organizationId, orgWithTightLimit.userId);
