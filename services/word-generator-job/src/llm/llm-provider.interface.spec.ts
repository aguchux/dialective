...
describe('parsePosItemArray', () => {
  it('parses a bare JSON array of {text, partOfSpeech} items', () => {
    expect(parsePosItemArray('[{"text": "cat", "partOfSpeech": "NOUN"}]')).toEqual([
      { text: 'cat', partOfSpeech: 'NOUN' },
    ]);
  });

  it('parses an { items: [...] } wrapper', () => {
    expect(parsePosItemArray('{"items": [{"text": "run", "partOfSpeech": "VERB"}]}')).toEqual([
      { text: 'run', partOfSpeech: 'VERB' },
    ]);
  });
  ...
  it('coerces an unrecognized partOfSpeech to OTHER instead of failing the batch', () => {
    expect(parsePosItemArray('[{"text": "cat", "partOfSpeech": "gerund"}]')).toEqual([
      { text: 'cat', partOfSpeech: 'OTHER' },
    ]);
  });

  it('coerces a missing partOfSpeech to OTHER', () => {
    expect(parsePosItemArray('[{"text": "cat"}]')).toEqual([
      { text: 'cat', partOfSpeech: 'OTHER' },
    ]);
  });
  ...
  it('skips items with missing/empty text without failing the whole batch', () => {
    expect(
      parsePosItemArray(
        '[{"text": "cat", "partOfSpeech": "NOUN"}, {"partOfSpeech": "VERB"}, {"text": "  "}]',
      ),
    ).toEqual([{ text: 'cat', partOfSpeech: 'NOUN' }]);
  });
