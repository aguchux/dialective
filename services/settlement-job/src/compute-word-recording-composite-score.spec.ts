...
describe('computeWordRecordingCompositeScore', () => {
  it('produces the identical result computeCompositeScore already gives when asrMatch weight is 0 (the shipped default)', () => {
    ...
    const original = computeCompositeScore(realScore, noiseScore, qualityScore, livenessScore, weights, SCORE_RANGE);
  ...
  it('falls back to neutral 100 for a null asrMatchScore (ASR hasn\'t landed yet)', () => {
    ...
    const result = computeWordRecordingCompositeScore(decimal(60), null, null, null, null, weights, SCORE_RANGE);
    // (60*50 + 100*50) / 100 = 80
  ...
  it('blends a real asrMatchScore proportionally to its weight', () => {
    const weights = { consensus: 50, noise: 0, quality: 0, liveness: 0, asrMatch: 50 };
    const result = computeWordRecordingCompositeScore(decimal(100), null, null, null, decimal(0), weights, SCORE_RANGE);
  ...
  it('clamps into [scoreRange.min, scoreRange.max]', () => {
    ...
    const result = computeWordRecordingCompositeScore(decimal(5), null, null, null, null, weights, { min: 10, max: 30 });
  ...
  it('falls back to the raw score when every weight (including asrMatch) is 0', () => {
    ...
    const result = computeWordRecordingCompositeScore(decimal(42), decimal(0), decimal(0), decimal(0), decimal(0), weights, SCORE_RANGE);
