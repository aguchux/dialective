...
### Word library (`frontend/app/WordLibraryFlow.tsx`, `words/`)
A second no-auth landing-page flow (toggle via the "Switch to word library" button in `page.tsx`), separate from the sentence-prompt pipeline test above and **not** wired to the ASR pipeline at all: `GET /api/v1/words/random` returns one English word from a fixed seed list (`prisma/seed.ts`, run via `npm run prisma:seed` — not admin-editable, adding a word means adding it to that list and re-seeding) → trainer types the translation in their chosen dialect and records themselves saying it → `POST /words/recordings/upload-url` + direct PUT to Spaces (same presign pattern as `submissions/upload-url`, same `SPACES_WORD_RECORDINGS_BUCKET`) → `POST /words/recordings` writes a `WordRecording` row straight to Postgres (`wordId`, `dialectTag`, `translationText`, `audioBucket`, `audioKey`). No Redis stream, no vosk/whisper worker, no transcript, no consensus scoring — the typed translation is trusted as-is. This is real production data (unlike the sentence-prompt flow's Redis scratch store), just gathered through a no-auth flow like the rest of the pipeline test.
...
- **Coinbase webhook signature verification requires the raw request body** (HMAC-SHA256 over the exact bytes, not the JSON-parsed object) — see `main.ts`'s middleware that path-checks `req.path === '/api/v1/wallet/webhooks/coinbase'` and only then runs a raw-body-preserving `json()` parser ahead of Nest's default one. **Do not use `app.use(path, middleware)` for this** — on Nest's Express adapter this does not scope by path (it still runs for every request and drains the body stream before Nest's own parser sees it), which silently broke `class-validator` on every other POST route in the app the first time this was tried. Always check `req.path` inside a single global middleware instead.
- **Secrets/config:** `coinbase-creds` (`coinbase_commerce_api_key`, `coinbase_commerce_webhook_secret`) + `TOKEN_USD_RATE`/`MIN_WITHDRAWAL_TOKENS` in `api-config`, same gitignored-`.env`-with-committed-`.example` pattern as every other secret/config in this repo.
- **Not built yet (explicit non-goals of the current implementation):** automated crypto payout API integration, P2P token escrow/marketplace (business plan §4 step 8), KYC/AML identity verification flow, multi-chain address selection (delegated entirely to Coinbase Commerce's checkout UI).
...
- `hf-creds` (`hf_token`) is consumed only by `whisper-worker` as `HF_TOKEN`. `NCAIR1/Igbo-ASR`, `NCAIR1/Yoruba-ASR`, and `NCAIR1/Hausa-ASR` are **gated repos** on Hugging Face — the model pages are publicly visible but anonymous `from_pretrained()`/`pipeline()` download gets a `401 GatedRepoError`, not a 404. This only surfaces at runtime (first job for that dialect_tag, since checkpoints download lazily) — nothing in `docker build` or `kubectl apply` catches it. Create a token at huggingface.co/settings/tokens (read access is enough) on an account that has requested and been granted access to all three model pages; `transformers` picks up `HF_TOKEN` from the environment automatically, no code change needed.
...
- `prisma-migrate.yml` validates `services/api`'s Prisma schema against a real `postgres:16-alpine` service container on every push/PR touching `services/api/prisma/**` or `src/**`: `prisma generate` → apply `prisma/migrations/` via `migrate deploy` → `prisma migrate diff --from-migrations ... --to-schema ... --exit-code` (Prisma 7 renamed this flag from `--to-schema-datamodel`, and now requires `datasource.shadowDatabaseUrl` set in `prisma.config.ts` — CI creates a `dialective_shadow` DB on the same Postgres service container and points `SHADOW_DATABASE_URL` at it before this step) to fail the build on any schema/migration drift → `npm run build`. If `prisma/migrations/` is empty (bootstrap case) it runs `prisma migrate dev --name init --create-only` and commits the generated migration back to the branch (`[ci skip]`, push-triggered runs only, never on PRs) — this is a one-time bootstrap path, not something that should fire repeatedly; once migrations exist, don't rely on CI to auto-generate future ones — run `prisma migrate dev` locally against a real Postgres and commit the result like any other schema change. The workflow has a `concurrency` group (queued, not cancelled) so two runs on `main` can't both see an empty `migrations/` dir and race to bootstrap it, plus the commit step fetches/rebases/retries before pushing — without both, two pushes landing close together produced exactly this race once (`43b3483` succeeded, a second run's push was rejected with "fetch first" since `main` had moved) — its migration.sql was lost, harmless since the file was reproducible, but don't strip the concurrency guard or the push retry back out.
