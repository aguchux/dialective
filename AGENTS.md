...
- **Funding (in):** `POST /wallet/deposits` creates a `pending` `Deposit` and a NOWPayments hosted invoice (`NowPaymentsService.createInvoice`, `order_id` set to `Deposit.id`), returns the invoice URL for the frontend to redirect to. The balance is **not** credited at this point — only `POST /wallet/webhooks/nowpayments` (NOWPayments' IPN callback on `payment_status: 'finished'`) credits it, inside a transaction, and is idempotent on replay (checks `Deposit.status` first). Never trust the client-side checkout redirect alone to credit funds — the invoice UI can show "paid" before the IPN confirms it server-side.
...
- **Secrets/config:** `nowpayments-creds` (`nowpayments_api_key`, `nowpayments_ipn_secret`) + `TOKEN_USD_RATE`/`MIN_WITHDRAWAL_TOKENS`/`API_PUBLIC_BASE_URL` in `api-config`, same gitignored-`.env`-with-committed-`.example` pattern as every other secret/config in this repo. `API_PUBLIC_BASE_URL` (default `https://api.nmseprep.com`) is used to build the `ipn_callback_url` sent with every invoice-creation request.
