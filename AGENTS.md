...
3. ✅ **Redis Streams topology + reliability layer** — `asr-jobs`, `consensus-jobs`, and `prompt-audio-jobs` streams with consumer groups; retry/DLQ mechanics (reclaim stuck pending entries, move poison messages to a `-dead` stream after N attempts) implemented in `RedisStreamsService` (NestJS) and `streams.py` (Python) — see "Redis Streams Reliability" below.
4. ✅ **vosk-worker (Python)** — consumes `asr-jobs` from Redis Streams via consumer group `asr-workers`; pre-filter → transcribe → publish-to-`consensus-jobs` flow implemented. `write_result` is a stub pending Postgres (step 2).
...
9. ✅ **K8s manifests** — Deployments for `api`/`consensus-scorer`/`prompt-audio-service`/`vosk-worker`, HPAs for `api`/`consensus-scorer`, KEDA ScaledObjects for `vosk-worker` and `prompt-audio-service` (both scaling on Redis Streams pending-entries-count, per the original design doc §6.2), CronJob for `settlement-job`, StatefulSets for Postgres and Redis, pgAdmin + Ingress, `api` + Ingress.
...
│   │   ├── prompt-audio-service-keda.yaml   # scales on Redis Streams pending-entries-count
...
- ASR, consensus scoring, and prompt audio generation are asynchronous batch workers consuming from **Redis Streams** (`asr-jobs`, `consensus-jobs`, `prompt-audio-jobs`) via consumer groups (`XREADGROUP`). Do not introduce synchronous HTTP calls between the API and a worker for scoring or TTS — the 12–24h settlement window (and, for prompt audio, "generate ahead of when a trainer needs it") means this pipeline is intentionally decoupled.
...
- **Dead-lettering:** after N failed attempts (e.g., 3), stop retrying — `XADD` the message to a companion dead stream (`asr-jobs-dead`, `consensus-jobs-dead`) with the failure reason, then `XACK` the original entry off the live stream so it stops being reclaimed.
- **Applies per stream:** `asr-jobs` (consumed by `vosk-worker`), `consensus-jobs` (consumed by `consensus-scorer`), and `prompt-audio-jobs` (consumed by `prompt-audio-service`) all need this — implement it as one shared piece of logic (a small reclaim/DLQ loop) reused across consumer groups rather than duplicated per service.
...
- Autoscaling for queue-driven workers (`vosk-worker`, `prompt-audio-service`) uses **KEDA on Redis Streams pending-entries-count**, not CPU-based HPA — CPU utilization is the wrong signal for this workload (design doc §6.2). Autoscaling for steady-load HTTP services (`consensus-scorer`, `api`) uses standard HPA on CPU.
...
- `docker-publish.yml` builds and pushes all 5 services (`api`, `consensus-scorer`, `prompt-audio-service`, `settlement-job`, `vosk-worker`) to Docker Hub as `golojan/dialect-<service>:latest` and `:<sha>`, on push to `main` (path-filtered per service + `models/`, since `vosk-worker`/`prompt-audio-service` depend on it) or manual dispatch. **Build context is per-service, not uniform** — `api`/`consensus-scorer`/`settlement-job`'s Dockerfiles `COPY` everything relative to their own service directory, so their matrix entry uses `context: services/<service>`; `vosk-worker`/`prompt-audio-service` additionally `COPY models/*.yaml` from outside their own directory, so those two use `context: .` (repo root) instead — get this wrong (e.g. repo-root context for all 5) and the 3 service-relative Dockerfiles fail with `"/src": not found` since `COPY src ./src` resolves against the wrong root. `fail-fast: false` on the matrix so one service's build failure doesn't cancel the other 4 (learned this the hard way — a `consensus-scorer` context bug once cancelled all 5 builds instead of only failing 1). Requires `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` repo secrets.
- `prisma-migrate.yml` validates `services/api`'s Prisma schema against a real `postgres:16-alpine` service container on every push/PR touching `services/api/prisma/**` or `src/**`: `prisma generate` → apply `prisma/migrations/` via `migrate deploy` → `prisma migrate diff --from-migrations ... --to-schema-datamodel ... --exit-code` to fail the build on any schema/migration drift → `npm run build`. If `prisma/migrations/` is empty (bootstrap case) it runs `prisma migrate dev --name init --create-only` and commits the generated migration back to the branch (`[ci skip]`, push-triggered runs only, never on PRs) — this is a one-time bootstrap path, not something that should fire repeatedly; once migrations exist, don't rely on CI to auto-generate future ones — run `prisma migrate dev` locally against a real Postgres and commit the result like any other schema change. The workflow has a `concurrency` group (queued, not cancelled) so two runs on `main` can't both see an empty `migrations/` dir and race to bootstrap it, plus the commit step fetches/rebases/retries before pushing — without both, two pushes landing close together produced exactly this race once (`43b3483` succeeded, a second run's push was rejected with "fetch first" since `main` had moved) — its migration.sql was lost, harmless since the file was reproducible, but don't strip the concurrency guard or the push retry back out.
