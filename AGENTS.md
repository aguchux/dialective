...
| Community forum         | **Discourse** (self-hosted, off-cluster) | `community.dialectlibrary.com` — DigitalOcean droplet run via `discourse_docker`'s `launcher`, **not** a k8s Deployment. See "Community (Discourse)" below.                                                                                                                                                                                                                                                          |
...
| Email                    | **Resend**                               | `api`'s `MailService` (`services/api/src/mail/`) sends password-reset, email-verification, and magic-link emails from `noreply@dialectlibrary.com`. Falls back to logging the link if `RESEND_API_KEY` is unset (local dev). Also used as the SMTP relay for Discourse.                                                                                                                                            |
...
│   │   ├── api/auth/[...nextauth]/, api/discourse-sso/  # NextAuth route handler; Discourse SSO bridge
...
├── discourse-droplet/                 # Discourse deployment runbook + app.yml.example (NOT a k8s manifest)
├── discourse-theme/                   # Discourse theme component matching the main app's palette
└── tests/
```
...
- **`whisper-worker`** handles languages Vosk has no model for — currently Igbo (`ig`), Yoruba (`yo`), Hausa (`ha`), via NCAIR1's Whisper-small fine-tunes (`NCAIR1/Igbo-ASR`, `NCAIR1/Yoruba-ASR`, `NCAIR1/Hausa-ASR` — Awarri Technologies' N-ATLaS initiative). Plain Hugging Face `transformers` pipeline, not `faster-whisper`/CTranslate2 (the checkpoints are `transformers`-format; the CPU-inference speed difference isn't worth the extra conversion step for short prompt-length clips).
- Checkpoints are **not** baked into the `whisper-worker` image — `cache_dir="/models"` on `whisper-model-repo-pvc` downloads lazily on first use, same pattern as `prompt-audio-service`'s TTS checkpoints.
...
- Secret/config groups now span far more than the original auth/spaces/postgres set — current `secrets/` includes `postgres`, `spaces`, `pgadmin`, `auth`, `hf`, `llm`, `nowpayments`, `flutterwave`, `flutterwave-v4`, `payout-crypto`, `didit`, `kyc-crypto`, `sms`, `rabbitmq`, `livekit`, `stream`, `discourse`, `discourse-postgres`; `configs/` includes per-service non-sensitive config for `api`, `postgres`, `pgadmin`, `vosk-worker`, `whisper-worker`, `quality-gate-worker`, `prompt-audio-service`, `isvc-scorer`, `audio-retention-job`, `chatdialect-agent`, `discourse`, `discourse-postgres`. If a manifest needs a new secret key, add it to the relevant `*.env.example` (or add a new `<name>.env.example` + generator entry).
...
- Ingress-exposed via this repo's k8s: `pgadmin` (`pgadmin.dialectlibrary.com`), `api` (`api.dialectlibrary.com`), `rabbitmq` UI (`rabbitmq.dialectlibrary.com`), `livekit` (provisioned, scaled to zero). `frontend` is **not** k8s-exposed — deployed to Vercel. `community.dialectlibrary.com` (Discourse) is **not** k8s-exposed either — DNS points directly at the standalone droplet, bypassing the cluster ingress controller entirely.
...
- Required env vars on Vercel (project settings, not `.env`/`secretGenerator`): `NEXTAUTH_URL`, `NEXTAUTH_SECRET`, `API_BASE_URL` (public `https://api.dialectlibrary.com` — Vercel can't reach the cluster's internal Service DNS), `NEXT_PUBLIC_API_BASE_URL`, `OAUTH_CALLBACK_SECRET` (must match `api`'s `auth-creds` value exactly), `DISCOURSE_SSO_SECRET`/`DISCOURSE_URL` (for the Discourse SSO bridge, see below).
- Whatever domain Vercel serves `frontend` on must be in `api`'s `CORS_ALLOWED_ORIGINS`.

### Community (Discourse)

`community.dialectlibrary.com` is self-hosted Discourse run via `discourse_docker`'s `launcher` on a dedicated DigitalOcean droplet — **not** a k8s Deployment. Discourse's own image only boots correctly via `launcher`'s bootstrap process; there's no maintained plain-Docker image that starts from bare env vars. `discourse-droplet/` holds the version-controlled template (`app.yml.example`) and the provisioning runbook (`discourse-droplet/README.md`) — the real `app.yml` with filled-in secrets lives only on the droplet, never in this repo. DNS for `community.dialectlibrary.com` points directly at the droplet's IP, bypassing the k8s ingress controller.

**SSO:** DiscourseConnect (Discourse's official SSO protocol) is bridged through `frontend/app/api/discourse-sso/route.ts` — Discourse never stores a password for any Dialect Library user; every login round-trips through this route, which trusts the existing NextAuth session, verifies/signs the DiscourseConnect payload with `DISCOURSE_SSO_SECRET` (HMAC-SHA256, must match the droplet's `app.yml` value exactly), and maps `role === 'ADMIN'` to both Discourse `admin` and `moderator` flags.

**Theme:** `discourse-theme/` is a git-based Discourse theme component (installed via Admin → Customize → Themes → Install → From a git repository, subfolder `discourse-theme`) matching the main app's palette (Inter font, same accent colors as `frontend/app/globals.css`) via Discourse's color-scheme mechanism, not hardcoded CSS.
...
- Self-hosting Discourse in this repo's Kubernetes cluster — it runs on its own droplet via `launcher`, deliberately outside kustomize's resource graph.
...
- `discourse-droplet/README.md` — Discourse provisioning runbook.
