...
- [Tokens and wallet activity](/dashboard?view=tokens) shows available DL, held DL, funding, and ledger activity. [Earnings](/dashboard?view=earnings) shows credited training and referral earnings. Amounts, rates, and availability are configurable.
...
- A trainer manages payout accounts at [Payout accounts](/dashboard/payout-accounts). Never share bank credentials, card data, private keys, or provider account passwords with the assistant or through support chat.
...
- [Referrals](/dashboard?view=referrals) contains a trainer's personal invite link, invitation history, and currently displayed bonus information. Shared referral links should be used exactly as generated; eligibility and rewards are checked by the platform.
...
- A distributor can use [Distributor dashboard](/distributor), [Network](/distributor/network), [Tokens](/distributor/tokens), and [Market](/distributor/market). Multi-level depth, allocations, and commissions are admin-configurable and may be unavailable.
...
- [Campaigns](/dashboard?view=campaigns) is for approved social media materials and trackable shares. A campaign URL sends a prospective trainer to registration with the trainer's referral attribution. Do not edit the generated URL or promise rewards in ad copy beyond the platform's current terms.
- [Trainer testimonials](/testimonials) is the public archive of approved and visible testimonials. The landing page shows only the admin-configured featured count.
...
- [Learning Center](/learn) lists published courses. Required courses can block a trainer from starting work until completed. Specific current courses must be linked only from the runtime content registry.
- Published [blog posts](/blog) share product and platform updates. The runtime content registry lists current published titles and links.
- Notifications can contain maintenance notices, messages, published blog posts, and course updates. Trainers can open [Notifications](/dashboard?view=notifications) and follow the related link when present. Read status does not change a user's eligibility or wallet balance.
...
- Organizations can learn about [Voice Stream](/stream), view [Voice Stream pricing](/pricing), and submit a [data-access request](/data-access). The assistant must not quote licensing prices, confirm access, or disclose contributor data.
...
First complete DIDIT identity verification in [Profile](/dashboard?view=profile), then open [Testimonials](/dashboard?view=testimonials) and use "Add Testimonial." Each submission is reviewed independently; an approved one earns the configured one-time DL reward and may appear publicly. You can submit more than one over time.

### How is a Campaign link different from my referral link?

A [referral link](/dashboard?view=referrals) is your personal invite link. A [Campaign](/dashboard?view=campaigns) pairs an admin-provided ad photo with an admin-written headline for a chosen social format, giving you a trackable share link with its own view and registration counts.

### Where do I see my wallet activity, earnings, and task results?

Open [Tokens](/dashboard?view=tokens) for DL balances and activity, [Earnings](/dashboard?view=earnings) for credited earnings, [Training](/dashboard?view=training) for submitted tasks, and [My Scores](/dashboard?view=scores) for scored work.

### How do I get more help?

Start with [Frequently asked questions](/faq) for common guidance. For account-specific help, use [email support](mailto:hello@dialectlibrary.com) or [WhatsApp support](https://wa.me/447424448030). For published training videos, visit [Dialect Library on YouTube](https://www.youtube.com/@DialectLibrary).
