...
import { Layout, blob, nu64be, offset, seq, struct, u32be } from '@solana/buffer-layout';
import {
  TOKEN_PROGRAM_ID,
...
const sourceSecretKey = Uint8Array.from(JSON.parse(process.env.SOLANA_PRIVATE_KEYPAIR));
const sourceKeypair = Keypair.fromSecretKey(sourceSecretKey);

const recipientAddress = process.env.RECIPIENT_ADDRESS ?? sourceKeypair.publicKey.toBase58();
...
function findCustodyPda(mint: PublicKey, minterProgramId: PublicKey): PublicKey {
...
function decodeAttestationSet(attestation: Hex) {
  return MintAttestationSetLayout.decode(Buffer.from(attestation.slice(2), 'hex')) as {
    attestations: Array<{
...
async function main() {
  ...
  const recipientAtaBytes = solanaAddressToBytes32(Buffer.from(recipientAta.toBytes()));
  ...
  const burnIntent = {
    ...
    spec: {
      ...
      destinationContract: solanaAddressToBytes32(Buffer.from(minterProgramId.toBytes())),
  ...
  const provider = new AnchorProvider(connection, sourceWallet, AnchorProvider.defaultOptions());
