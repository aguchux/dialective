...
async function main() {
  ...
  const recipientAta = getAssociatedTokenAddressSync(destinationMint, solanaWallet.publicKey);
  ...
  const burnIntent = {
    ...
    spec: {
      ...
      sourceContract: evmAddressToBytes32(SOURCE_CHAIN.gatewayWallet as `0x${string}`),
